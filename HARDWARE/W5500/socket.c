...
uint16 send(SOCKET s, const uint8 * buf, uint16 len)
{
  ...
  while ( (IINCHIP_READ(Sn_IR(s) ) & Sn_IR_SEND_OK) != Sn_IR_SEND_OK )
  {
    ...
    status = IINCHIP_READ(Sn_SR(s));
              if ((status != SOCK_ESTABLISHED) && (status != SOCK_CLOSE_WAIT))
              {
                printf("SEND_OK Problem!! Current status: %d\r\n", status);
                switch (status) {
                  case SOCK_INIT:
                    printf("Socket is in INIT state.\r\n");
                    break;
                  case SOCK_LISTEN:
                    printf("Socket is in LISTEN state.\r\n");
                    break;
                  case SOCK_SYNSENT:
                    printf("Socket is in SYN_SENT state.\r\n");
                    break;
                  case SOCK_SYNRECV:
                    printf("Socket is in SYN_RECEIVED state.\r\n");
                    break;
                  case SOCK_CLOSING:
                    printf("Socket is in CLOSING state.\r\n");
                    break;
                  case SOCK_TIME_WAIT:
                    printf("Socket is in TIME_WAIT state.\r\n");
                    break;
                  case SOCK_CLOSED:
                    printf("Socket is in CLOSED state.\r\n");
                    break;
                  default:
                    printf("Unknown socket state.\r\n");
                    break;
                }
                close(s);
                return 0;
